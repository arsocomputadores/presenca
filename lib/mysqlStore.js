...
function getBimestreDateRange(ano, mesInicio) {
  const year = Number(ano);
  const startMonth = Number(mesInicio);
  const endMonth = startMonth === 12 ? 1 : startMonth + 1;
  const endYear = startMonth === 12 ? year + 1 : year;
  const inicio = `${year}-${String(startMonth).padStart(2, '0')}-01`;
  const lastDay = new Date(endYear, endMonth, 0).getDate();
  const fim = `${endYear}-${String(endMonth).padStart(2, '0')}-${String(lastDay).padStart(2, '0')}`;
  return { inicio, fim, mesFim: endMonth, anoFim: endYear };
}

let turmaHorarioColumnsReady = false;
let turmaHorarioSemanaTableReady = false;
let alertaFrequenciaAutoTableReady = false;
let usuarioPodeConsultarOutrasTurmasColumnReady = false;
...
async function ensureUsuarioPodeConsultarOutrasTurmasColumn() {
  if (usuarioPodeConsultarOutrasTurmasColumnReady) return;
  const pool = await getPool();
  const hasColumn = await tableColumnExists(pool, 'usuarios', 'pode_consultar_outras_turmas');
  if (!hasColumn) {
    await pool.query(`
      ALTER TABLE usuarios
      ADD COLUMN pode_consultar_outras_turmas TINYINT(1) NOT NULL DEFAULT 0 AFTER ativo
    `);
  }
  usuarioPodeConsultarOutrasTurmasColumnReady = true;
}
...
async function getRelatorioFrequenciaTurmaDetalhado(turmaId, mes, ano) {
  const pool = await getPool();
  const { inicio, fim } = getBimestreDateRange(ano, mes);
...
async function getTurmasProfessorHorario(usuarioId, horario = 1, dataStr = null) {
  ...
  const [rows] = await pool.query(`
    ...
    LEFT JOIN turma_professores_horario tph
      ...
      AND tph.dia_semana <=> ?
    WHERE t.ativa = 1
      AND COALESCE(tph.professor_id, t.${coluna}) = ?
    ORDER BY t.nome
  `, [horario, diaSemana, usuarioId]);
  return aplicarEscalaSemanaNasTurmas(pool, rows);
}
...
async function usuarioPodeLancarHorarioNaTurma(usuarioId, turmaId, horario = 1, dataStr = null) {
  ...
  `, [horario, diaSemana, turmaId]);
  if (!row) return false;
  return Number(row.professor_horario_id || 0) === Number(usuarioId);
}

async function getUsuarios() {
  const pool = await getPool();
  await ensureUsuarioPodeConsultarOutrasTurmasColumn();
  const [rows] = await pool.query(`
...
async function cadastrarUsuario({ nome, cpf, senha, perfil, ativo, turmas_ids, pode_consultar_outras_turmas }) {
  const pool = await getPool();
  await ensureUsuarioPodeConsultarOutrasTurmasColumn();
  const conn = await pool.getConnection();
  try {
    ...
    const [res] = await conn.query(
      'INSERT INTO usuarios (nome, cpf, senha_hash, perfil, ativo, pode_consultar_outras_turmas) VALUES (?, ?, ?, ?, ?, ?)',
      [nome, cpf, senha, perfil, ativo ?? 1, Number(pode_consultar_outras_turmas) ? 1 : 0]
    );
...
async function atualizarUsuario(id, { nome, cpf, senha, perfil, ativo, turmas_ids, pode_consultar_outras_turmas }) {
  const pool = await getPool();
  await ensureUsuarioPodeConsultarOutrasTurmasColumn();
  const conn = await pool.getConnection();
  try {
    ...
    let sql = 'UPDATE usuarios SET nome = ?, cpf = ?, perfil = ?, ativo = ?, pode_consultar_outras_turmas = ?';
    const params = [nome, cpf, perfil, ativo, Number(pode_consultar_outras_turmas) ? 1 : 0];
