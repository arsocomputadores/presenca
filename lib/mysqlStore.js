...
async function getTurmas(usuarioId = null) {
  ...
  let sql = `
    SELECT t.*, (SELECT COUNT(*) FROM matriculas m WHERE m.turma_id = t.id AND m.ativa = 1) as total_alunos 
    FROM turmas t 
  `;
  const params = [];
...
async function cadastrarTurma({ nome, serie, turno, ano_letivo }) {
  const pool = await getPool();
...
async function atualizarTurma(id, { nome, serie, turno, ano_letivo }) {
  const pool = await getPool();
  await pool.query(
    'UPDATE turmas SET nome = ?, serie = ?, turno = ?, ano_letivo = ? WHERE id = ?',
    [nome, serie, turno, ano_letivo, id]
  );
}
...
async function getAlunosProjeto(projetoId) {
  ...
  const [rows] = await pool.query(`
    ...
    LEFT JOIN matriculas m ON a.id = m.aluno_id AND m.ativa = 1
    LEFT JOIN turmas t ON m.turma_id = t.id
    WHERE pa.projeto_id = ? AND pa.ativo = 1
  `, [projetoId]);
...
async function salvarFrequencia(turmaId, dataStr, registros, usuarioId, horario = 1) {
  ...
  try {
    ...
    await conn.query('DELETE FROM frequencias WHERE turma_id = ? AND data = ? AND horario = ?', [turmaId, dataStr, horario]);
...
async function getDashboardStats(usuarioId = null, perfil = 'admin') {
  ...
  const freqParams = [new Date().toISOString().slice(0, 10)];
...
async function criarMensagemInterna({ remetente_id, titulo, corpo, tipo_destino, perfil_destino, usuario_ids }) {
  const pool = await getPool();
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
