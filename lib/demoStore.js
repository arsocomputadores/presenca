...
function getTurmasProfessorHorario(usuarioId, horario = 1, dataStr = null) {
  ...
  return data.turmas
    ...
    .filter((t) => {
      ...
      return Number(responsavel) === Number(usuarioId);
    })
...
function usuarioPodeLancarHorarioNaTurma(usuarioId, turmaId, horario = 1, dataStr = null) {
  ...
  const professorId = diaSemana ? turma.escala_semana?.[diaSemana]?.[horario]?.professor_id : null;
  const fallbackId = Number(horario) === 6 ? turma.professor_sexto_horario_id : turma.professor_primeiro_horario_id;
  return Number(professorId || fallbackId || 0) === Number(usuarioId);
}
...
function listSolicitacoesEdicaoFrequenciaPendentes() {
  const data = loadDemo();
  return data.frequencia_solicitacoes_edicao
    .filter((item) => item.status === 'pendente')
    .map((item) => getSolicitacaoEdicaoFrequenciaById(item.id))
    .sort((a, b) => {
      const dataCmp = String(a.criado_em || '').localeCompare(String(b.criado_em || ''));
      if (dataCmp !== 0) return dataCmp;
      const turmaCmp = String(a.turma_nome || '').localeCompare(String(b.turma_nome || ''), 'pt-BR');
      if (turmaCmp !== 0) return turmaCmp;
      return Number(a.horario || 1) - Number(b.horario || 1);
    });
}
...
function negarEdicaoFrequencia(solicitacaoId, adminId) {
  const data = loadDemo();
  const item = data.frequencia_solicitacoes_edicao.find((row) => row.id === Number(solicitacaoId));
  if (!item || item.status !== 'pendente') {
    throw new Error('Solicitação não encontrada ou já foi tratada.');
...
function listSolicitacoesLancamentoForaHorarioPendentes() {
  const data = loadDemo();
  return data.frequencia_solicitacoes_lancamento
    .filter((item) => item.status === 'pendente')
    .map((item) => getSolicitacaoLancamentoForaHorarioById(item.id))
...
function negarLancamentoForaHorario(solicitacaoId, adminId) {
  const data = loadDemo();
  const item = data.frequencia_solicitacoes_lancamento.find((row) => row.id === Number(solicitacaoId));
  if (!item || item.status !== 'pendente') {
    throw new Error('Solicitação não encontrada ou já foi tratada.');
...
function getRelatorioFrequenciaTurma(turmaId, mes, ano) {
  const alunos = getAlunos({ turma_id: turmaId, ativo: 1 });
  const data = loadDemo();
  const inicio = new Date(ano, mes - 1, 1);
  const fim = new Date(ano, mes, 0);
  ...
  return alunos.map((aluno) => {
    const freqs = data.frequencias.filter((f) => {
      if (Number(f.turma_id) !== Number(turmaId)) return false;
      if (Number(f.aluno_id) !== Number(aluno.id)) return false;
      const d = new Date(`${f.data}T12:00:00`);
      return d >= inicio && d <= fim;
    });
    ...
    return {
      ...
      faltas,
      justificadas,
      total_aulas: freqs.length,
    };
  });
}

function getHistoricoIndividual(alunoId, ano, mes) {
  const data = loadDemo();
  const inicio = new Date(ano, mes - 1, 1);
  const fim = new Date(ano, mes, 0);

  return data.frequencias
    .filter((f) => {
      if (Number(f.aluno_id) !== Number(alunoId)) return false;
      const d = new Date(`${f.data}T12:00:00`);
      return d >= inicio && d <= fim;
    })
    .map((f) => ({
    ...
    .sort((a, b) => {
      ...
      if (dataCmp !== 0) return dataCmp;
      return Number(b.horario) - Number(a.horario);
    });
}

function getHistoricoIndividualPorPeriodo(alunoId, inicio, fim) {
  const { normalizeHistoricoFrequenciaRow } = require('./reportUtils');
  const data = loadDemo();
  const ini = new Date(`${inicio}T12:00:00`);
  const fi = new Date(`${fim}T12:00:00`);
  const usuariosMap = new Map((data.usuarios || []).map((u) => [Number(u.id), u.nome]));
  const turmasMap = new Map((data.turmas || []).map((t) => [Number(t.id), t.nome]));
  ...
  return data.frequencias
    ...
    .map((f) => normalizeHistoricoFrequenciaRow({
      data: f.data,
      horario: Number(f.horario || 1),
      status: f.status,
      observacao: f.observacao || '',
      lancado_por_nome: f.lancado_por ? usuariosMap.get(Number(f.lancado_por)) || null : null,
      turma_nome: turmasMap.get(Number(f.turma_id)) || null,
    }))
    .sort((a, b) => {
      const dataCmp = String(b.data).localeCompare(String(a.data));
      if (dataCmp !== 0) return dataCmp;
...
function getResumoEstatisticasAluno(alunoId, ano) {
  const data = loadDemo();
  const registrosAno = data.frequencias.filter((f) => {
    if (Number(f.aluno_id) !== Number(alunoId)) return false;
    const d = new Date(`${f.data}T12:00:00`);
  ...
  const geral = {
    presencas: registrosAno.filter((f) => f.status === 'P').length,
    faltas: registrosAno.filter((f) => f.status === 'F').length,
    justificadas: registrosAno.filter((f) => f.status === 'J').length,
    total: registrosAno.length,
  };

  const graficoMap = new Map();
  registrosAno.forEach((f) => {
    const d = new Date(`${f.data}T12:00:00`);
    const mesNum = d.getMonth() + 1;
    if (!graficoMap.has(mesNum)) graficoMap.set(mesNum, 0);
    if (f.status === 'F') graficoMap.set(mesNum, graficoMap.get(mesNum) + 1);
  });

  const grafico = [...graficoMap.entries()]
    .map(([mesNum, faltas]) => ({ mes: mesNum, faltas }))
    .sort((a, b) => a.mes - b.mes);

  return { geral, grafico };
}
...
function getDirecaoLancamentoUsuariosPermitidos() {
  const data = loadDemo();
  const ids = Array.isArray(data.configuracoes.direcao_lancar_frequencia_usuarios)
    ? data.configuracoes.direcao_lancar_frequencia_usuarios
    : [];
  return [...new Set(ids.map((v) => Number(v)).filter((n) => Number.isFinite(n) && n > 0))];
}

function setDirecaoLancamentoUsuariosPermitidos(usuarioIds) {
  const data = loadDemo();
...
function usuarioDirecaoPodeLancarFrequencia(usuarioId) {
  const id = Number(usuarioId);
  if (!Number.isFinite(id) || id <= 0) return false;
  const permitidos = getDirecaoLancamentoUsuariosPermitidos();
